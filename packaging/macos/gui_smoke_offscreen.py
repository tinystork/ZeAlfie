...
def main(argv: list[str] | None = None) -> int:
    ...
    isolated_root = Path(tempfile.mkdtemp(prefix="zealfie-macos-gui-smoke-runtime-"))
    ...
    exit_code = app.exec()
    if exit_code != 0:
        print(f"QT_SMOKE FAILED: event loop exited rc={exit_code}", file=sys.stderr)
        return 1

    print(
        "QT_SMOKE=PASS "
        f"title={window.windowTitle()!r} interpreter={sys.executable}"
    )
    return 0
